module ApplicationHelper
  ...
  def user_id(user)
    ...
    if user.email.present? && user.name.present?
      tags << gravatar_image_tag(user.email, :alt => user.name, :class => 'user-gravatar', :gravatar => { :size => 50 })
      tags << user.email
    elsif user.email.present?
      tags << gravatar_image_tag(user.email, :class => 'user-gravatar', :gravatar => { :size => 50 })
      tags << user.email
    end
    tags.html_safe
  end
