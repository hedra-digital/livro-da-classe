...
namespace :sletras do
  desc "connect to remote server"
  task :connect do
    Kernel.system("ssh -i ~/.ssh/livrodaclasse_rsa deploy@7letras.livrodaclasse.com.br")
  end

  desc "tail 7letras log"
  task :log do
    Kernel.system("ssh -i ~/.ssh/livrodaclasse_rsa deploy@7letras.livrodaclasse.com.br 'tail -f ~/apps/livrodaclasse/current/log/sletras.log'")
  end

  desc "returns a 7letras console"
  task :console do
    Kernel.exec("ssh -i ~/.ssh/livrodaclasse_rsa deploy@7letras.livrodaclasse.com.br  'cd ~/apps/livrodaclasse/current/; bundle exec rails c sletras'")
  end

  desc "updates XML sitemap"
  task :sitemap do
    Kernel.exec("ssh -i ~/.ssh/livrodaclasse_rsa deploy@7letras.livrodaclasse.com.br 'cd ~/apps/livrodaclasse/current/; rake sitemap:refresh'")
  end

  desc "remakes search index and restarts Sphinx"
  task :search_rebuild do
    Kernel.exec("ssh -i ~/.ssh/livrodaclasse_rsa deploy@7letras.livrodaclasse.com.br 'cd ~/apps/livrodaclasse/current/; rake ts:rebuild'")
  end
end
